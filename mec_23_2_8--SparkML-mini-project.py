...
# MAGIC %sql
# MAGIC SELECT DISTINCT education FROM adult
...
# Select the required columns and create the bachelor_rate on the fly
df_result = df_adult.select(
  df_adult['education'],
  when(col('marital_status') == ' Never-married', 1).otherwise(0).alias('is_bachelor')
)

# Group by education and calculate the mean of the is_bachelor column, then round it
df_result = df_result.groupBy('education').agg(round(mean('is_bachelor'), 2).alias('bachelor_rate'))

# Order the results by bachelor_rate in descending order
...
# evaluate the performance using an evaluator:
...
# display predictions
display(gbtPredictions.select("label", "prediction", "probability"))
...
def print_performance_metrics(predictions):
    ...
    # Positive label to be passed to multi_metrics methods
    pcl = 1.0
  
    # F1
    print("F1 = {}".format(multi_metrics.fMeasure(label=pcl)))
    # Precision
    print("Precision = {}".format(multi_metrics.precision(label=pcl)))
    # Recall
    print("Recall = {}".format(multi_metrics.recall(label=pcl)))
...
print_performance_metrics(gbtPredictions)
...
# Create CrossValidator
gbCv = CrossValidator(estimator=gbt, estimatorParamMaps=lrParamGrid, evaluator=evaluator, numFolds=2)
...
[print((name, lrModel.coefficients[idx]), "\t") for idx, name in attrs]
...
for i, importance in enumerate(rfFeatureImportance):
